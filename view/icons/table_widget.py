...
class TableWidget(QWidget, ui_BuildTestTableWidget.Ui_BuildTestTableWidget):
    ...
    def __change_combo_box(self, row, col, text, change_func, isLoad=None):
        """
        콤보박스 선택 변경 시 Step 객체와 테이블 셀을 동기화합니다.
        """
        # Step 데이터 반영
        change_func(row, text, isLoad)

        # Sub Step의 TYPE 변경은 UI 반영만으로 충분 (추가 동작 불필요)
        if col == ColNum.TYPE.value and str(text) == "Sub":
            return

        # 테이블 셀 텍스트 업데이트 (콤보박스 선택값 표시)
        item = self.tableWidget.item(row, col)
        if item is not None:
            item.setText(text)
        else:
            self.new_cell(row, col, text=text, method_name=self.generate_value_cell)
    ...
    def __context_menu(self, position):
        menu = QMenu()
        copy_ = menu.addAction("Copy Step")
        paste_ = menu.addAction("Paste Step")
        menu.addSeparator()
        insert_ = menu.addAction("Insert Step")
        delete_ = menu.addAction("Delete Step")
        menu.addSeparator()

        action = menu.exec_(self.tableWidget.mapToGlobal(position))
        row = self.tableWidget.indexAt(position).row()

        if action == copy_:
            self.__copy_step(row)
        elif action == paste_:
            self.paste_step_above(row)
        elif action == insert_:
